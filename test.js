"use strict";
function mvStartCode() {

    function testFun() {
        // console.log('сработало наведение')
        if (!document.querySelector('[data-product-inv="0"]')) return
        const falsButton = document.querySelector('#rec665855420 a')//фальшивая ссылка в скрытой кнопке
        const productName = document.querySelector('[data-product-inv="0"] .t-store__prod-popup__title-wrapper .js-store-prod-name')//наименование продукта
        const variants = document.querySelectorAll('[data-product-inv="0"] input')//все инпуты
        const price = document.querySelector('[data-product-inv="0"] .t-container_fluid .js-product-price')//стоимость
        const fotoProduct = document.querySelector('[data-product-inv="0"] img')//первое фото из всех
        const fotoProductMobile = document.querySelectorAll('[data-product-inv="0"] [data-original]')[1]//первое фото из всех (мобильная версия)
        let variant;
        for (let i = 0; i < variants.length; i++) {
            if (variants[i].checked) {
                variant = `${variants[i].name}: ${variants[i].value} `
                console.log(variant)
                break
            }
            
        }
        falsButton.href = `#order:${productName.textContent} __ ${variant}(предзаказ)=${price.dataset.productPriceDefStr}:::image=${fotoProductMobile.dataset.original || fotoProduct.src }`
    }
    function addNalog() {//альтернатива дополнительной кнопке
        const product = {
          name: "Налог 6%",
          amount: getNalog(6),
          price: getNalog(6),
          img: "https://pluspng.com/img-png/taxes-icon-256.png",
        };
        tcart__addProduct(product);
        // tcart.products.push(product);
        // tcart__saveLocalObj()
        // tcart__reDrawProducts()
        tcart__updateTotalProductsinCartObj();
        tcart__reDrawCartIcon();
        tcart__reDrawTotal();
        tcart__saveLocalObj();
        tcart__reDrawProducts()
      }



    function butFun() {
        const falsButton = document.querySelector('#rec665855420 a')//фальшивая ссылка
        falsButton.click()
    }

    function createPredZakaz() {
        const button = document.querySelector('[data-product-inv="0"] .wbtn')//кнопки предзаказ
        button.addEventListener('mouseover', testFun)
        button.addEventListener('click', butFun)
        button.addEventListener('click', () => {
            // console.log('клик мобилка')
            testFun()
        })
    }


    let status = true;
    let startCode = setInterval(() => {
        if (!document.querySelector('[data-product-inv="0"] .wbtn')) {
            // console.log('ищем кнопку')
            status = true
            return
        }

        if (status === false) return;
        // console.log('нашли')

        createPredZakaz()
        status = false
        // clearInterval(startCode)
    }, 800)

}

document.addEventListener('DOMContentLoaded', () => {
    console.log('сработка')
    mvStartCode();
})
